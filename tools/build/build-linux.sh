...
APP_SRC=./tools/build/_app_scaffolds/linux/myapp.desktop
INSTALL_SCRIPT=./tools/build/_app_scaffolds/linux/install.sh
...
for APP_ARCH in "${APP_ARCH_LIST[@]}"; do
    ...
    if [ -e "./tools/build/preproc-linux.sh" ]; then
        echo "  Running pre-processor ..."
        . ./tools/build/preproc-linux.sh
    fi
